...
class DragAndDrop extends Component  {
  ...
  skipDropAreaClick = (e) => {
    document.addEventListener('click',(e) =>{
      if((e.target.className !== 'dropArea')/*|| (e.target.className !== 'dropArea_h1') ||(e.target.className !== 'dropArea_p')*/) {
          this.setState({dragging: false})
      }
    }, true
)
  ...
  render () {
    const {dragging} = this.state;
      return (
      <div className='dropArea_wrapper' ref={this.dropRef} onKeyDown={this.skipDropAreaEsc} onClick={this.skipDropAreaClick}>
        {dragging && 
