    ...
    return (
      <div className="sender">
        ...
        <form
          ...
          /* action="https://api.sendsay.ru/clu180" method="POST" onSubmit={this.handleSubmit}  encType="multipart/form-data"*/ autoComplete="off"
        >
          <label className="sender_label sender_from ">
            ...
            <input
              ...
              name="emailFrom"
              placeholder="Email"
              value={inputFrom.fromEmail}
              onChange={this.handleInput}
              onBlur={this.validateForm}
            />
            {from.validate && (
              <p className="sender_alertLeft">
                {from.placeholder} не может быть пустым
              </p>
            )}
            {emailFrom.validate && (
              <p className="sender_alertRight">
                {emailFrom.placeholder} не может быть пустым
              </p>
            )}
          </label>
          <label className="sender_label sender_to">
            ...
            <input
              ...
              name="emailTo"
              placeholder="Email"
              value={inputFrom.toEmail}
              onChange={this.handleInput}
              onBlur={this.validateForm}
            />
            {to.validate && (
              <p className="sender_alertLeft">
                {to.placeholder} не может быть пустым
              </p>
            )}
            {emailTo.validate && (
              <p className="sender_alertRight">
                {emailTo.placeholder} не может быть пустым
              </p>
            )}
          </label>
          <label className="sender_label ">
            Тема письма <br />
            <input
              className="sender_input sender_theme"
              type="text"
              name="theme"
              value={inputFrom.theme}
              onChange={this.handleInput}
            />
          </label>
          <label className="sender_label ">
            Сообщение <br />
            <textarea
              className="sender_input sender_msg"
              name="msg"
              value={inputFrom.msg}
              onChange={this.handleInput}
            />
